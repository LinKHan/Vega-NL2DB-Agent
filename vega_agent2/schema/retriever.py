"""Schema RAG retrieval for Vega Agent 2.

Responsibilities:
- Provide keyword/rule retrieval compatible with the original V9 project.
- Optionally use embedding + FAISS semantic retrieval for schema selection.
...
from vega_agent2.config import SCHEMA_RAG_TOP_K, SCHEMA_RETRIEVER_MODE
...
def retrieve_schema(question: str, history_context: list = None, forced_keys: list = None, top_k: int = 5) -> list:
    top_k = top_k or SCHEMA_RAG_TOP_K
    mode = SCHEMA_RETRIEVER_MODE
    if mode in {"embedding", "vector", "faiss"}:
        try:
            from vega_agent2.schema.embedding_retriever import retrieve_schema_by_embedding

            return retrieve_schema_by_embedding(question, history_context, forced_keys, top_k)
        except Exception:
            return retrieve_schema_by_keyword(question, history_context, forced_keys, top_k)

    if mode == "hybrid":
        return retrieve_schema_hybrid(question, history_context, forced_keys, top_k)

    return retrieve_schema_by_keyword(question, history_context, forced_keys, top_k)


def retrieve_schema_hybrid(question: str, history_context: list = None, forced_keys: list = None, top_k: int = 5) -> list:
    forced_keys = forced_keys or []
    embedding_items = []
    try:
        from vega_agent2.schema.embedding_retriever import retrieve_schema_by_embedding

        embedding_items = retrieve_schema_by_embedding(question, history_context, forced_keys, top_k)
    except Exception:
        embedding_items = []

    keyword_items = retrieve_schema_by_keyword(question, history_context, forced_keys, top_k)
    final_keys = []
    for item in embedding_items + keyword_items:
        key = item["key"]
...
def retrieve_schema_by_keyword(question: str, history_context: list = None, forced_keys: list = None, top_k: int = 5) -> list:
    forced_keys = forced_keys or []
